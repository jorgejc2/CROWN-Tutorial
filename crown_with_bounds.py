...
class BoundSequential(nn.Sequential):
    ...
    def _get_optimized_bounds(self, x_U=None, x_L=None, upper=False, lower=True, use_input_constraints=False):
        ...
        if use_input_constraints:
            ...
            # if lower is True:
            #     lambda_ = torch.tensor([[0.016700245106524528, 0.0, 0.0, 0.0, 0.014915326040875582], [0.0, 0.0, 0.0, 0.04031287734715239, 0.0]]).T
            # else:
            #     lambda_ = torch.tensor([[0.0, 0.0017826694885925691, 0.0, 0.0, 0.0], [0.0, 0.020208684089320802, 0.0, 0.0, 0.0]]).T
        ...
        if (use_input_constraints):
            ...
            for i in range(num_subs):
                ...
                if i == 0:
                    plt.title(f"Loss Function for {'Lower Bound' if upper is False else 'Upper Bound'}")
                    plt.ylabel(f"y")
...
if __name__ == '__main__':
    ...
    h = torch.tensor([[-46/9], [-6], [-25/3], [-19/4], [-24]])
    boundedmodel = BoundSequential.convert(model.model)

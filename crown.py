...
if __name__ == '__main__':
    ...
    device = torch.device('cpu') # JC use CPU for now
    print("device: {}".format(device))

    model = Model().to(device)
    # torch.save(model.state_dict(), 'model_verysimple.pth')
    model.load_state_dict(torch.load('model.pth')) # JC load in the model to get consisten outputs
